    ...
    return FutureBuilder(
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
          ...
          this._isFavorite = this
              ._favoritesData
              .teams
              .containsKey("${team.name} (${team.seasonId})");
          ...
          List<Widget> pages = <Widget>[
                  ...
                  children: <Widget>[
                    ...
                    Card(
                      ...
                      child: Column(
                        ...
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: <Widget>[
                          Text("${_averageGoals.toStringAsFixed(2)}",
                              style: Theme.of(context).textTheme.headline),
                          Text(
                            "Average Goals/Game",
                          ),
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  return ListTile(
                    ...
                    subtitle: team.roster[index].id == team.captainPlayerId
                        ? Text("Captain")
                        : null,
                    onTap: () {
            ...
            Center(
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  String win = ((team.schedule[index].homeTeamName ==
                                  team.name &&
                              int.parse(team.schedule[index].homeGoals) >
                                  int.parse(team.schedule[index].awayGoals)) ||
                          (team.schedule[index].awayTeamName == team.name &&
                              int.parse(team.schedule[index].awayGoals) >
                                  int.parse(team.schedule[index].homeGoals)))
                      ? "W"
                      : "L";
                  ...
                  return ListTile(
                    ...
                    title: Text(
                        "${team.schedule[index].homeTeamName} vs. ${team.schedule[index].awayTeamName}"),
                    subtitle: Text(
                        "${team.schedule[index].startTime} - ${team.schedule[index].rinkName}"),
                    trailing: team.schedule[index].played == "1"
                        ? Text(
                            "${team.schedule[index].homeGoals.toString()} - ${team.schedule[index].awayGoals.toString()}")
                        : Text("Unplayed"),
                    onTap: () {
                      Navigator.of(context).push(
                        MaterialPageRoute(
            ...
            child: PlatformScaffold(
              appBar: PlatformAppBar(
                ...
                trailingActions: <Widget>[
                  PlatformIconButton(
                    icon:
                        Icon(this._isFavorite ? Icons.star : Icons.star_border),
                    onPressed: () {
                      this.setState(() {
                      ...
                      if (this._isFavorite) {
                        _favoritesData
                            .teams["${team.name} (${team.seasonId})"] = team.id;

                        // DateFormat notificationDateFormat = DateFormat.jm();
                        // // Schedule match notifications
                        // for (int i = 0; i < team.schedule.length; i++) {
                        //   if (team.schedule[i].startTimeDateTime
                        //           .difference(DateTime.now())
                        //           .inMinutes >=
                        //       0) {
                        //     _broomballNotifications.scheduleNotification(
                        //         team.id,
                        //         team.schedule[i].startTimeDateTime
                        //             .subtract(Duration(minutes: 30)),
                        //         "${team.schedule[i].homeTeamName} vs. ${team.schedule[i].awayTeamName} - ${notificationDateFormat.format(team.schedule[i].startTimeDateTime)} - ${team.schedule[i].rinkName}");
                        //     print(
                        //         "Registered notification ${DateTime.now()} at ${team.schedule[i].startTimeDateTime.subtract(Duration(minutes: 30))}");
                        //   }
                        // }
                      } else {
                        _favoritesData.teams
                            .remove("${team.name} (${team.seasonId})");

                        // _broomballNotifications
                        //     .cancelNotificationsById(team.id);
                      }
